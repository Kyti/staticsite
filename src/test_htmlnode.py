...
class TestHTMLNode(unittest.TestCase):
    ...
    def test_leaf_to_html_no_value(self):
        node = LeafNode(None, "kittens")
        self.assertEqual(node.to_html(), "kittens")

    def test_leaf_to_html_success(self):
    ...
    def test_to_html_missing_children(self):
        # We need to manually set children to None after creation
        node = ParentNode("div", [])
        node.children = None
        with self.assertRaises(ValueError) as context:
            node.to_html()
